...
for i in range(len(t)): # Loop over all the t's, however, the for loop will never complete
    ...
    #print('Time: ' + str(t[i]))
    #print('Trace:' + str(tr_trim))

    
    # if the trim is next to the end, preload a new one 
    while tr_trim.stats.endtime > st_IC[-1].stats.endtime - pp['coarse_duration']:
        ...
        try:
            if data_source=='local':
                ...
                st.trim(starttime=t[i] - pp['coarse_duration'], endtime=st[-1].stats.endtime ) # trim off the part of the merged stream that's already been processed.
                print(st)
            skip_step = False 
