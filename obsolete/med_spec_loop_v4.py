...
config.read('/data/stor/proj/Turner/med_spec_output/run2020_2022/med_spec_TURNER_20s.par')
...
for i in np.arange(250, 270):#range(len(t)): # Loop over all the t's, however, the for loop will never complete
...
#                print('About to read in new datafile: ' + str(dt.datetime.now() - time_temp) )             
        ...
        # if the current stream is longer than 1.1 days, we are getting to a data gap:
        # this statement skips to the where there is renewed data if that is the case. 
        if st[-1].stats.endtime - st[0].stats.starttime > 1.1 * 24 * 3600:
            print('stream is longer than 1.1 days! at ' + str(t[i]))
            # locate the index in t of the end of the stream (which is where there is new data)
            if t < st[-1].stats.endtime: 
